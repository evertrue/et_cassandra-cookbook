shared_examples_for 'an opscenter installation' do |log_root, log_dir|
  describe 'DataStax OpsCenter' do
    ...
    it 'has world-readable logs' do
      expect(file("#{log_dir}/opscenter")).to be_mode '755'
    end

    context 'has content in the correct log files' do
      describe file("#{log_dir}/opscenter/opscenterd.log") do
        it { is_expected.to be_file }
        describe '#content' do
          subject { super().content }
